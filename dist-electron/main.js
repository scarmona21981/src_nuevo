...
process.on('uncaughtException', (error) => {
    ...
    electron_1.dialog.showErrorBox('Error de Aplicacion', error.message + '\n\n' + error.stack);
});
process.on('unhandledRejection', (reason, promise) => {
    console.error('UNHANDLED REJECTION:', reason);
});
function ensureSmalExtension(filePath) {
    const trimmed = filePath.trim();
    if (trimmed.toLowerCase().endsWith('.smal'))
        return trimmed;
    const ext = path_1.default.extname(trimmed);
    if (!ext)
        return `${trimmed}.smal`;
...
const popoutWindowMeta = new Map();
let popoutSeq = 1;
let latestProjectSnapshotJson = null;
let latestAnalysisSnapshotJson = null;
let pendingPopoutLayout = [];
let popoutRestoreDone = false;
let shouldPersistPopoutLayout = true;
let persistPopoutLayoutTimer = null;
let restorePopoutsOnStartup = true;
const POPOUT_LAYOUT_FILENAME = 'popout-layout.json';
const MAX_RESTORED_POPOUTS = 8;
const isPopoutView = (value) => {
...
function getPopoutLayoutPath() {
    return path_1.default.join(electron_1.app.getPath('userData'), POPOUT_LAYOUT_FILENAME);
}
async function readPopoutLayoutFromDisk() {
    const fallback = { popouts: [], restoreOnStartup: true };
    const layoutPath = getPopoutLayoutPath();
    try {
        const content = await promises_1.default.readFile(layoutPath, 'utf-8');
...
async function writePopoutLayoutToDisk(entries) {
    const layoutPath = getPopoutLayoutPath();
    const payload = JSON.stringify({ version: 1, restoreOnStartup: restorePopoutsOnStartup, popouts: entries }, null, 2);
    try {
        await promises_1.default.writeFile(layoutPath, payload, 'utf-8');
    }
    catch (error) {
        console.warn('[popout:layout] No se pudo guardar layout.', error);
    }
}
function writePopoutLayoutToDiskSync(entries) {
    const layoutPath = getPopoutLayoutPath();
    const payload = JSON.stringify({ version: 1, restoreOnStartup: restorePopoutsOnStartup, popouts: entries }, null, 2);
    try {
        fs_1.default.writeFileSync(layoutPath, payload, 'utf-8');
    }
    catch (error) {
        console.warn('[popout:layout] No se pudo guardar layout (sync).', error);
    }
}
function collectPopoutLayoutEntries() {
    const entries = [];
...
function bindPopoutLayoutPersistence(w) {
...
function buildWindowTarget(params) {
...
function sendProjectSnapshot(target, snapshotJson, sourceWindowId) {
...
function sendAnalysisSnapshot(target, snapshotJson, sourceWindowId) {
    if (target.isDestroyed())
        return;
...
async function openPopoutWindow(payload, options) {
    const view = isPopoutView(payload?.view) ? payload.view : 'resultados';
...
async function restorePopoutsFromLayout() {
    if (popoutRestoreDone)
        return;
    popoutRestoreDone = true;
    if (!restorePopoutsOnStartup)
        return;
    if (pendingPopoutLayout.length === 0)
        return;
...
function resolveProjectPathFromArgs(args) {
    for (const rawArg of args) {
        ...
        if (!normalizedArg)
            continue;
        const ext = path_1.default.extname(normalizedArg).toLowerCase();
...
function flushPendingProjectOpen() {
    if (!pendingProjectPath || !mainWindow || mainWindow.isDestroyed() || !rendererReady)
        return;
...
function createSplashWindow(onSplashShown) {
    ...
    if (process.env.VITE_DEV_SERVER_URL) {
        ...
        splashWindow.loadFile(path_1.default.join(__dirname, '../dist/splash.html'));
    }
...
electron_1.ipcMain.on('app:loading-progress', (_, data) => {
    if (splashWindow && !splashWindow.isDestroyed()) {
        splashWindow.webContents.send('app:loading-progress', data);
    }
});
electron_1.ipcMain.on('app:ready', () => {
    rendererReady = true;
    if (splashWindow && !splashWindow.isDestroyed()) {
        splashWindow.close();
    }
...
electron_1.ipcMain.handle('file:open', async () => {
    ...
    try {
        const rawBuffer = await promises_1.default.readFile(filePaths[0]);
...
electron_1.ipcMain.handle('file:load', async (_, filePath) => {
    try {
        const rawBuffer = await promises_1.default.readFile(filePath);
        const content = decodeProjectBuffer(rawBuffer);
...
electron_1.ipcMain.handle('file:save', async (_, { path, content }) => {
    await promises_1.default.writeFile(path, content, 'utf-8');
    return true;
});
electron_1.ipcMain.handle('file:save-as', async (_, content) => {
    ...
    await promises_1.default.writeFile(normalizedPath, content, 'utf-8');
    return normalizedPath;
});
electron_1.ipcMain.handle('popout:open', async (_evt, payload) => {
    return openPopoutWindow(payload);
});
electron_1.ipcMain.handle('popout:close', async (_evt, windowId) => {
...
electron_1.ipcMain.handle('popout:closeAll', async () => {
...
electron_1.ipcMain.handle('popout:list', async () => {
    return Array.from(popoutWindows.keys());
});
electron_1.ipcMain.handle('popout:getInit', async (_evt, windowId) => {
...
electron_1.ipcMain.handle('popout:getRestoreOnStartup', async () => {
    return restorePopoutsOnStartup;
});
electron_1.ipcMain.handle('popout:setRestoreOnStartup', async (_evt, enabled) => {
    restorePopoutsOnStartup = enabled !== false;
    await writePopoutLayoutToDisk(collectPopoutLayoutEntries());
    return restorePopoutsOnStartup;
});
electron_1.ipcMain.on('project:snapshot', (_evt, payload) => {
    ...
    latestProjectSnapshotJson = payload.snapshotJson;
    for (const [id, w] of popoutWindows.entries()) {
        if (id === sourceWindowId || w.isDestroyed())
            continue;
...
electron_1.ipcMain.handle('project:getLatestSnapshot', async () => {
    return latestProjectSnapshotJson;
});
electron_1.ipcMain.on('analysis:snapshot', (_evt, payload) => {
...
electron_1.ipcMain.handle('analysis:getLatestSnapshot', async () => {
    return latestAnalysisSnapshotJson;
});
electron_1.ipcMain.handle('app:exit', () => {
    electron_1.app.quit();
});
// Handle unsaved changes response from renderer
electron_1.ipcMain.on('window:unsaved-changes-response', async (_, hasChanges) => {
    if (!mainWindow)
        return;
    if (hasChanges) {
...
electron_1.ipcMain.on('window:save-completed', () => {
    if (mainWindow) {
        mainWindow.destroy();
        electron_1.app.quit();
    }
